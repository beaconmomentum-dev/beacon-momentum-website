...
Both diagnostic routes retain the same “Built by Beacon Labs / Visit Beacon Momentum” proof-of-capability handoff. Their footers identify Beacon Labs as “a Beacon Momentum, LLC company,” explicitly list the wider Beacon ecosystem, and link to Beacon Momentum.[2] [3]
...
That makes Beacon Momentum a **reference implementation**, not a direct service line of Beacon Labs. The overlap is nonetheless real: some Beacon Momentum members may be operators, coaches, consultants, or course creators. A cross-link therefore needs an eligibility boundary and consentful referral workflow instead of a blanket Labs upsell to every member.
...
> **Initial assessment:** Beacon Labs is best understood as the B2B engineering and implementation arm of the ecosystem. Beacon Momentum is a live internal proof point and a separate member-facing product. Their relationship should be deliberately connected as **case study → qualified B2B inquiry**, while the two offers, audiences, pricing, accounts, and compliance responsibilities remain distinct.
...
## Current Beacon Momentum constraints

The current Beacon Momentum architecture already establishes a useful division of labor. **The Watch** is one annual, member-facing community with earned progression rather than separately purchasable product tiers. **Beacon Venture** teaches members to validate, launch, and systematize their own offers. **Beacon Systems** is the organizational implementation and consultation bridge. The current **Beacon Labs** member pillar is explicitly framed as the research-and-proof layer: Signal Check methodology, controlled experiments, case studies, and applied playbooks.

That means Beacon Labs should not be folded into The Watch as another paid tier, a default member benefit, a replacement community, or a general-purpose upsell. Doing so would blur a consumer/member commitment with a bespoke B2B implementation engagement and compromise the single-membership doctrine.

## Recommended ecosystem placement

> **Decision: connect Beacon Labs to Beacon Momentum as a separate B2B implementation arm; do not fold its sales, payment, intake, CRM, or client delivery into The Watch.**

| Ecosystem element | Primary job | Relationship to Beacon Labs | Recommended public treatment |
|---|---|---|---|
| **The Watch / Beacon Momentum** | Member transformation, practical curriculum, community, and earned progression | Serves as a tested reference implementation and optional source of qualified founder/operator referrals | Retain the existing member experience. Do not put Labs service pricing, paid Blueprint checkout, or lead forms inside the Watch. |
...
| **Beacon Labs — AI Infrastructure Engineering** | B2B diagnostic, strategy, implementation, and retained operations | Separate legal, sales, payments, data, delivery, and support surface | Retain `beaconlabs.ai` as its own domain and service funnel. Treat Beacon Momentum as a published proof point, not a product catalog. |
...
External business operator
        │
        ├── Beacon Labs Signal Check → paid Growth Blueprint → Beacon Labs service engagement
        │                                  (independent B2B funnel, consent, payment, and CRM)
        │
        └── “See the system in the field” → Beacon Momentum case study / public story
                                                   │
                                                   └── Optional Watch enrollment if the person seeks
                                                       personal operating practice and community

Existing Watch member / Beacon Venture operator
        │
        └── Beacon Systems fit discussion → explicit optional referral → Beacon Labs
                                             (new intake; no member-data transfer or prefill)
```

This is deliberately a **two-way proof-and-referral relationship**, not a merged customer journey. Beacon Labs can cite Beacon Momentum as a live field implementation. Beacon Momentum can offer a narrow, opt-in bridge for an established operator who needs organization-level installation. Neither side should auto-enroll, auto-tag, or expose the other side’s records.

## Required conversion and brand cleanup before linking more traffic

| Priority | Required change | Why it matters |
|---|---|---|
| **1 — Resolve the Signal Check offer** | Establish one canonical offer ladder: for example, a clearly scoped **free Signal Snapshot**, a separately priced **full Signal Check**, then the $997 **Growth Blueprint**. Alternatively, retain only the paid version. Remove contradictory “free” and “$497” descriptions. | A controlled referral cannot send people into a diagnostic whose price, depth, and delivery promise conflict across routes. |
...
| **1 — Clarify the public relationship** | Replace generic ecosystem links with two contextual proof links: Labs → “See Beacon Momentum in the field,” and Momentum → “Explore optional business implementation through Beacon Labs.” | External links open a distinct Labs journey; they do not transfer form values, account IDs, or audience tags. | Visitors can explain the difference between membership and B2B services. |
| **2 — Controlled referral** | Add a Beacon Systems consultation question: “Are you seeking organization-level implementation support?” If yes, present an explicit referral choice to Labs. | The user starts a new Labs intake and consents there. Use only a source parameter such as `source=beacon-momentum`; do not prefill data. | Qualified, consented referrals are attributable without merged profiles. |
| **3 — Credible case study** | Publish a validated Beacon Momentum implementation story inside Labs, and an educational “How the system was tested” resource inside the member Labs pillar. | Separate marketing claims from member curriculum; disclose the shared ownership relationship. | A reusable, source-backed proof asset on both domains. |
...
The public review now confirms the service funnels, public payment/intake route names, major customer segments, and current public legal-route gap. It does **not** establish the actual CRM, report-generation engine, data stores, payment-provider configuration, whether direct path access is correctly validated server-side, or whether any existing Beacon Momentum member records are present in Labs systems. Those questions require authorized technical and business-owner review before Phase 0 is marked complete.
